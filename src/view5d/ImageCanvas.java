...
public void paint(Graphics g) {
	...
	g.drawRect(0, 0, r.width-1, r.height-1);   // draw the frame
        ...
        /*
         * Draw all visible ROIs - Connie Jiang
         */
        g.setColor(Color.RED);
        ...
        for(int i = 0; i < my3ddata.roiList.list.size(); i++) {
        	RectROI rROI; 
        	
        	rROI = (RectROI) my3ddata.roiList.list.elementAt(i);
        	
        	float pos[] = myPanel.getPositions();
        
        	
        	//IJ.log("Cross Hair Location (" + pos[0] + ", " + pos[1] + ", " + pos[2] + ")");
        	
        	if(rROI.InROIRange((int)pos[0], (int)pos[1], (int)pos[2], DimNr)) {
        		Rectangle r2 = rROI.GetSqrROI(DimNr);
                int xrs = DMouseFromX(r2.x + ROIMoveDX);
                int yrs = DMouseFromY(r2.y + ROIMoveDY);
                int xre = DMouseFromX(r2.x+r2.width + ROIMoveDX);
                int yre = DMouseFromY(r2.y+r2.height + ROIMoveDY);

                int tmp;
                if (xrs > xre) {tmp=xrs;xrs=xre;xre=tmp;}
                if (yrs > yre) {tmp=yrs;yrs=yre;yre=tmp;}
                g2D.drawRect(xrs, yrs, xre-xrs, yre-yrs);
                
                g2D.drawString(""+(i+1), xrs, yrs + fm.getAscent());
            }
        } 
...
public void ProcessKey(char myChar) {
   ...
   case 'Q':  // remove all point to the end of the list
	...
	//repaint();
	//return ;
	// Above were commeted out in original distribution, not by Connie Jiang
	// Adding ROI support using ':' for removal and ';' for intertionn - by Connie Jiang
    case ';':
    	//Inserting an ROI
    	ROI roi = my3ddata.BundleAt(my3ddata.ActiveElement).ActiveROI;
    	((RectROI)roi).classify(my3ddata);
    	my3ddata.roiList.Insert(roi);
    	
    	IJ.log("ROI List:\n" + my3ddata.roiList.toString());
    	return;
    case ':':
    	//Removing an ROI
    	float pos[] = myPanel.getPositions();
    	if(my3ddata.roiList.SetActiveROI(pos))
    		my3ddata.roiList.Remove();
    	IJ.log("Current ROI List: " + my3ddata.roiList.toString());
    	return;
